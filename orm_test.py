...
def update_user_test(type_, path, user, updated_user):
    db = get_db(type_, path)
    db.create_user(user)
    db.update_user(user.name, params)
    db_user = db.get_user(user.name, 'work')
...
def main():
    ...
    updated_sqlite_user = User(1, 'test', '30', '30', '30', '4')
    updated_json_user= User(None, 'test', '30', '30', '30', '4')
